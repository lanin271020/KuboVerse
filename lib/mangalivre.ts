...
/**
 * Segunda fonte de dados, complementar à MangaDex — só manhwa e mangá shonen. Site brasileiro
...
/**
 * Curadoria desta fonte no KuboVerse: só manhwa (coreano) OU mangá shonen.
 * Manhua e demais demografias de mangá (seinen, josei, etc.) ficam de fora.
 */
const REGEX_SHONEN = /shou?nen/i;

function temSinalShonen(
  generos: string[],
  tagsBrutas: string[],
  sinalMangaDex: { demograficoShonen?: boolean } | null
): boolean {
  if (sinalMangaDex?.demograficoShonen) return true;
  if (generos.some((g) => REGEX_SHONEN.test(g))) return true;
  if (tagsBrutas.some((tag) => REGEX_SHONEN.test(tag))) return true;
  return false;
}
